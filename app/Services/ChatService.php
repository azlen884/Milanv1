...
class ChatService {
    ...
    /**
     * Send voice message with strict server-side daily message limit enforcement
     */
    public static function sendVoiceMessage(
        int $senderId,
        int $conversationId,
        string $mediaUrl,
        int $durationSeconds
        ...
        // STRICT DAILY LIMIT CHECK (Rule 20: voice messages count as messages for Free user's daily limit)
        $perm = Auth::canSendMessage($senderId);
        if (!$perm['allowed']) {
            return [
                'success' => false,
        ...
        $msgId = Database::insert(
            "INSERT INTO messages (conversation_id, sender_id, receiver_id, type, media_url, duration_seconds, is_read, created_at) 
             VALUES (:cid, :sid, :rid, 'voice', :url, :dur, 0, NOW())",
            [
                ':cid' => $conversationId,
                ':sid' => $senderId,
                ':rid' => $receiverId,
                ':url' => $mediaUrl,
                ':dur' => max(1, $durationSeconds),
            ]
        );
        ...
        Database::insert(
            "INSERT INTO notifications (user_id, type, title, body, link) 
             VALUES (:uid, 'message', 'New Voice Message', :body, :link)",
            [
                ':uid' => $receiverId,
                ':body' => "{$sender['name']} sent you a voice message.",
                ':link' => "/chat?id={$conversationId}",
            ]
        );
